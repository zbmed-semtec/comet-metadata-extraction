...
def test_cli_extract_invokes_service_and_prints_json(capsys, monkeypatch):
    monkeypatch.setattr(sys, "argv", ["comet-rs", "extract", "https://github.com/a/b", "connoss", "--token", "t"])
    with patch("app.cli.run_extraction", return_value=({"@type": "Software"}, {"name":{"source":"X","confidence":1,"category":"recommended"}})) as m:
        main()
    out = json.loads(capsys.readouterr().out)
    assert out["code_url"].endswith("/a/b") and out["results"] == {"@type": "Software"} and m.called
...
def test_cli_extract_property_exits_1_when_no_match(capsys, monkeypatch):
    ...
    with patch("app.cli.run_extraction", return_value=({"other": 1}, {})):
...
def test_cli_extract_property_success_emits_flat_output(capsys, monkeypatch):
    ...
    with patch("app.cli.run_extraction", return_value=({"name": "X"}, {"name":{"source":"API","confidence":.5,"category":"recommended"}})):
        main()
    out = json.loads(capsys.readouterr().out)
...
def test_cli_gitlab_url_picks_gitlab_token_from_env(capsys, monkeypatch):
    ...
    with patch("app.cli.run_extraction", return_value=({"name": "X"}, {"name":{"source":"API","confidence":1,"category":"recommended"}})) as m:
        main()
